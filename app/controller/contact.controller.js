...
exports.create = async (req, res, next) => {
  if (!req.body.name) {
    return next(new BadRequestError(400, "Name can't be empty"));
  }
  ...
  if (error) {
    return next(new BadRequestError(500,
      "An error occurred while crreating the contact"));
  }
...
exports.findAll = async (req, res, next) => {
  ...
  if (error) {
    return next(new BadRequestError(500, "An error occurred while retrieving "))
  }
...
exports.findOne = async (req, res, next) => {
  ...
  const [error, documents] = await handle(Contact.findOne(condition));
...
exports.update = async (req, res, next) => {
  ...
  const [error, document] = await handle(Contact.findOneAndUpdate(condition, req.body, {
    new: true,
  })
  );
...
exports.delete = async (req, res, next) => {
  ...
  if (error) {
    return next(new BadRequestError(500, `Couldn't delete contact with id = ${req.params.id} `));
  }
...
exports.deleteAll = async (rea, res, next) => {
